...
//!
//! - [`FileConfig`] — POD struct tree mirroring `scrap.toml`
...
//!   with `meta.config_file_name` — the config-file literal (the
//!   unified `scrap.toml`, shared by every adapter) lives in the binary
//!   crate, never in scrap-core.
...
//!   the loader to find `scrap.toml` at the workspace root, not
...
//! `exclude = [...]` entry in user-authored `scrap.toml` files
...
// ────────────────────────────────────────────────────────────────────────
// POD-type re-exports (relocated to `domain::config` per scrap-rs#21
// cabinet MF-1 fold)
//
// The four POD schema types — `FileConfig`, `OptOutPolicy`,
// `DetectorConfig`, `Override` — moved to `crates/scrap-core/src/domain/config.rs`
// so `detectors/` and `core/` can depend on them without violating
// adr-hexagonal-layout (detectors/ and core/ may NOT depend on cli/).
// The loader pipeline below (`load_config`, `discover_config`,
// `resolve_detector_for_path`, `ConfigError`, private RawConfig
// mirrors, validation helpers) stays here — those are CLI-edge
// concerns (I/O + diagnostics + validation).
//
// Re-exports keep `use scrap_core::cli::config::FileConfig` compiling
// for existing external consumers without a flag-day update; new code
// is encouraged to import directly from `domain::config`.
// ────────────────────────────────────────────────────────────────────────

pub use crate::domain::config::{
    DetectorConfig, FileConfig, OptOutPolicy, Override, resolve_detector_for_path,
};
...
// ────────────────────────────────────────────────────────────────────────
// (POD types `FileConfig`, `OptOutPolicy`, `DetectorConfig`, `Override`
// moved to `crates/scrap-core/src/domain/config.rs` per scrap-rs#21
// cabinet MF-1 fold. Re-exported at the top of this module for
// external-consumer compat. POD-shape tests moved alongside.)
//
...
/// match.
///
/// **Adapter-name-agnostic API**: `file_name` is the config-file
/// literal — the unified `scrap.toml`, shared by every adapter —
/// supplied at call time via `meta.config_file_name` from the binary
/// crate. This module never references that name
...
/// expect the loader to find `scrap.toml` at the workspace root,
...
// ────────────────────────────────────────────────────────────────────────
// (Canonical overrides resolver `resolve_detector_for_path` relocated
// to `crate::domain::config` in scrap-rs#21 cabinet MF-1 fold so
// detectors/ + core/ can call it without violating
// adr-hexagonal-layout. Re-exported at the top of this module for
...
mod tests {
    ...
    // ── POD-shape tests RELOCATED to `crate::domain::config::tests`
    //    per scrap-rs#21 cabinet MF-1 fold (default_config_is_empty_pod,
    //    default_round_trips_to_empty_toml,
    //    btreemap_smellcategory_key_{deserializes,serializes}_snake_case,
    //    override_match_field_renames_correctly,
    //    opt_outs_omitted_deserializes_to_none,
    //    opt_outs_honor_empty_serializes_and_reparses_to_some_empty,
    //    opt_outs_honor_with_variants_round_trips).
